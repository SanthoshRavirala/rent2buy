		...
		<form class="form-horizontal" action="<?php echo $link;?>" method="post" enctype='multipart/form-data'>		 
			...
			<div class="form-group<?php echo (isset($error['error_page_id'])) ? " has-error" : "";?>">
			 ...
			 <div class="form-group<?php echo isset($error['error_name']) ? " has-error" : "";?>">
			  ...
			  <div class="col-sm-10">
				<?php $meta_datas = $this->db->get_where('gallery',array('gallery_id'=>$id,'tb_name'=>'sell_a_car'))->result_array();
                      if(!empty($meta_datas)){				 
				 ?>
				<?php  foreach($meta_datas as $meta_data){ ?>
				
				<div class="col-md-4" style="margin-right:50px;">
					<a href="#" class="" data-href="<?php echo site_url('admin/sell_a_car/delete_image/'.$meta_data['id'].'/'.$meta_data['gallery_id']);?>" data-toggle="modal" data-target="#confirm-delete">
	...
	<?php }  }?>
			 ...
			 <label for="input" class="col-sm-2 control-label"><?php echo 'Included List';?></label>
				...
				<table id="myTable" class=" table order-list">
				    ...
				    <tbody>
				    	<?php 
							$tab_datas=$this->db->get_where('sell_a_car',array('id'=>$id))->result_array();
							if(!empty($tab_datas)) { 
		  ...
		  <!-- /.box-footer -->
		   <input type="hidden" name="<?php echo 'car_type'; ?>" value="<?php echo 'sell_a_car'; ?>">
