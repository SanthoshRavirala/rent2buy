...
class Rent_a_car extends CI_Controller {
	...
	{ 	
		...
		$var_header_data['title'] = 'Rent a car';			   	           
	    ...
	    $data['users'] = array();	    
		$user_number = $this->db->get('rent_a_car')->num_rows();				
	    $users = $this->db->get('rent_a_car', $limit, $start)->result_array();
		
		foreach($users as $user){
			$data['users'][] =  array(
				'id'   => $user['id'],
				'title'   => $user['title'],
				'link'    => site_url('admin/rent_a_car/add/'.$user['id']),
			    'link_clone'        => site_url('admin/rent_a_car/rent_a_car_clone/'.$user['id']),
			    'delete_link' => site_url('admin/rent_a_car/delete/'.$user['id'])  ,
			);            
		}
		...
		$this->load->view('admin/rent_a_car', $data);
	}
	...
	public function add($id = 0, $post = array(), $error = array())
	{
		...
		if($id && !$post){
			$post = $this->db->get_where('rent_a_car', array('id' => $id))->row_array();
		}
		if($post){
		...
		$data['error'] = $error;	
	    if($id){
			$data['link'] = site_url('admin/rent_a_car/validate/'.$id);
		} else {
			$data['link'] = site_url('admin/rent_a_car/validate');
		}
		
	    $data['cancel'] = site_url('admin/rent_a_car');
		$data['id'] = $id;
		$data['header'] = $this->load->view('admin/header', array_merge($fixed_header_data, $var_header_data), true);
        $data['footer'] = $this->load->view('admin/footer', $this->layout->get_footer_data('user'), true);
		$data['sidebar'] = $this->load->view('admin/sidebar', $this->layout->get_left_data('user'), true);
		
		$this->load->view('admin/rent_a_car_add', $data);
	}
	public function validate($id = 0){
		...
		$linkCount = $this->db->get_where('rent_a_car',array('link'=> $slug))->result_array();
		if(!empty($linkCount))
		{
		...
		$image2 = $this->input->post('image2');
					...
					{
						$uploads_dir = 'uploads/rent_a_car';
		...
		if($error){
			...
			if($id){
				...
				$this->db->update('rent_a_car', $post, 'id = '.$id);
                ...
                for($i = 0; $i < $filesCount; $i++){ 
						...
						$data_file = array('image' => 'uploads/gallery/' .$ramod_numvv.'-'. $filename,'gallery_id'=>$id,'tb_name'=>'rent_a_car');
				...
				$post['list_1']=json_encode($post['list_1']);
				$post['list_2']=json_encode($post['list_2']);
				$this->db->insert('rent_a_car', $post);
                ...
                for($i = 0; $i < $filesCount; $i++){ 
						  ...
						  // Upload file
						  move_uploaded_file($_FILES['files']['tmp_name'][$i],'./uploads/gallery/'.$ramod_numvv.'-'.$filename);
						$data_file = array('image' => 'uploads/gallery/' . $ramod_numvv.'-'.$filename,'gallery_id'=>$insert_id,'tb_name'=>'rent_a_car');
						 $this->db->insert('gallery', $data_file);
					}
				} 
	   		
			}
			redirect('admin/rent_a_car');			
		}
	}
	
	public function rent_a_car_clone($id = null)
	{	
		
		if($id){
			$rent_a_car = $this->db->get_where('rent_a_car', array('id' => $id))->row_array();
		...
		$this->db->insert('rent_a_car',$rent_a_car_data);
			$insert_id=$this->db->insert_id();
	...
	{	
		...
		$this->db->where('tb_name','rent_a_car');
		$this->db->delete('gallery');
		$this->db->delete('rent_a_car', 'id = '.$id);
		redirect('admin/rent_a_car');
	}
	...
	public function delete_image($id = 0,$gallery_id=0, $token)
	{
		...
		redirect('admin/rent_a_car/add/'.$gallery_id);
	}
}