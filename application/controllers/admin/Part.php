...
class Part extends CI_Controller {
	...
	{ 	
		$this->lang->load('admin/part', $this->language->get_name());					
		$this->load->library('admin/layout');
				
		$fixed_header_data = $this->layout->get_header_data('part');
		...
		$data['link_add'] = site_url('admin/part/add');
		$limit = $this->config->item('c_limit');
		
	    $data['parts'] = array();	    
		$part_number = $this->db->get('part')->num_rows();				
	    $parts = $this->db->get('part', $limit, $start)->result_array();
		
		foreach($parts as $part){
			$data['parts'][] =  array(
				'part_id' => $part['part_id'],
				'name'        => $part['name'],
				'link'        => site_url('admin/part/add/'.$part['part_id'])	,
				'delete_link' => site_url('admin/part/delete/'.$part['part_id'])	,
			);            
		}
		
		if($part_number < $start + $limit){
			$to = $part_number;
		} else {
			$to = $start+ $limit;
		}
		...
		if($part_number){
			$data['text_showing'] = sprintf($this->lang->line('text_showing'),$start + 1, $to, $part_number);
		} else {
		...
		$p_config['total_rows'] = $part_number;		
        ...
        $data['footer'] = $this->load->view('admin/footer', $this->layout->get_footer_data('part'), true);
		$data['sidebar'] = $this->load->view('admin/sidebar', $this->layout->get_left_data('part'), true);
	...
	public function add($id = 0, $post = array(), $error = array())
	{
		...
		if($id && !$post){
			$post = $this->db->get_where('part', array('part_id' => $id))->row_array();
		}
		if($post){
		...
		$data['error'] = $error;	
	    if($id){
			$data['link'] = site_url('admin/part/validate/'.$id);
		} else {
			$data['link'] = site_url('admin/part/validate');
		}
		
	    $data['cancel'] = site_url('admin/part');
		
		$data['header'] = $this->load->view('admin/header', array_merge($fixed_header_data, $var_header_data), true);
        $data['footer'] = $this->load->view('admin/footer', $this->layout->get_footer_data('part'), true);
		$data['sidebar'] = $this->load->view('admin/sidebar', $this->layout->get_left_data('part'), true);
		
		$this->load->view('admin/part_add', $data);
	}
	public function validate($id = 0){
		$this->lang->load('admin/part_error', $this->language->get_name());
		$error = array();
		...
		if($error){
			...
			if($id){
				$this->db->update('part', $post, 'part_id = '.$id);
			} else {
	...
	{	
		...
		$this->db->delete('part', 'part_id = '.$id);
		redirect('admin/part');
	}
}