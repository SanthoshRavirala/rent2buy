...
class Rent_to_buy_booking extends CI_Controller {
	...
	{ 	
		...
		$var_header_data['title'] = 'Rent to buy booking';			   	           
	    ...
	    $data['users'] = array();	    
		               $this->db->where('book_type','Rent to buy');
		...
		foreach($users as $user){
			$data['users'][] =  array(
			    ...
			    'delete_link' => site_url('admin/rent_to_buy_booking/delete/'.$user['id'])  ,
			);            
		}
		...
		$this->load->view('admin/rent_to_buy_booking', $data);
	}
	...
	{	
		...
		redirect('admin/rent_to_buy_booking');
	}
}