...
class Sell_a_car_booking extends CI_Controller {
	...
	{ 	
		...
		$var_header_data['title'] = 'Sell a car booking';			   	           
	    ...
	    $data['users'] = array();	    
		               $this->db->where('book_type','Sell a car');
		...
		foreach($users as $user){
			$data['users'][] =  array(
			    ...
			    'delete_link' => site_url('admin/sell_a_car_booking/delete/'.$user['id'])  ,
			);            
		}
		...
		$this->load->view('admin/sell_a_car_booking', $data);
	}
	...
	{	
		...
		redirect('admin/sell_a_car_booking');
	}
}